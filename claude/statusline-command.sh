...
# Single line: Model | Folder | Branch | Progress % | Cost | Time | Cache
status_line=""

# Model (bold, white)
status_line=$(printf '%b%s%b' "$BOLD" "$short_model" "$RESET")

# Folder (bright blue)
status_line="$status_line $(printf '%b \033[94m%s\033[0m' "$SEP" "$folder_name")"

# Branch (cyan with git icon)
if [ -n "$git_branch" ]; then
    status_line="$status_line $(printf '%b \033[96m⎇ %s\033[0m' "$SEP" "$git_branch")"
fi

# Progress bar with percentage (if available)
if [ -n "$progress_bar" ]; then
    status_line="$status_line $(printf '%b %b %b%s%b' "$SEP" "$progress_bar" "$DIM" "$ctx_pct" "$RESET")"
fi

# Cost (yellow/gold)
status_line="$status_line $(printf '%b \033[33m$%s\033[0m' "$SEP" "$cost")"

# Session time (cyan)
if [ -n "$session_time" ]; then
    status_line="$status_line $(printf '%b \033[36m%s\033[0m' "$SEP" "$session_time")"
fi
