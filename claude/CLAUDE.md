...
Check for `uv.lock` → use `uv run`/`uv add`/`uv sync` (do not use `pip install` or activate manually). Otherwise check for `.venv` → prefix commands with `source .venv/bin/activate &&`. If neither exists, use the `setup-venv` skill.
